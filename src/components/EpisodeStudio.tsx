...
export default function EpisodeStudio({ storyBible, episodeNumber, previousChoice, storyBibleId, isRewrite = false }: EpisodeStudioProps) {
  ...
  // Rewrite episode functionality
  const [allEpisodes, setAllEpisodes] = useState<Record<number, any>>({})
  const [isRewriting, setIsRewriting] = useState(false)
  const [rewriteEpisodeNumber, setRewriteEpisodeNumber] = useState<number | null>(null)
  ...
  // Load all episodes for rewrite functionality
  useEffect(() => {
    const loadAllEpisodes = async () => {
      ...
      try {
        ...
        console.log(`✅ Loaded ${Object.keys(episodes).length} episodes for rewrite functionality`)
      } catch (error) {
  ...
  // Auto-generate content when in rewrite mode
  useEffect(() => {
    const performRewrite = async () => {
      if (!isRewrite || hasAutoGenerated || !storyBible || !storyBibleId) return
      if (Object.keys(allEpisodes).length === 0) return // Wait for episodes to load
      
      const episodeToRewrite = allEpisodes[episodeNumber]
      if (!episodeToRewrite) {
        console.warn(`Episode ${episodeNumber} not found for rewrite`)
        return
      }

      console.log(`🔄 Auto-generating rewrite content for Episode ${episodeNumber}...`)
      setHasAutoGenerated(true)
      setIsRewriting(true)
      setRewriteEpisodeNumber(episodeNumber)

      try {
        // Get all previous episodes (excluding the one being rewritten)
        ...
        const prevEp = episodeNumber > 1 ? allEpisodes[episodeNumber - 1] : null
        ...
        if (analysisData.success && analysisData.settings) {
          ...
          // Apply the AI-analyzed settings (ensure strings)
          setVibeSettings(analyzedVibes)
          setDirectorsNotes(typeof analyzedNotes === 'string' ? analyzedNotes : String(analyzedNotes || ''))
          setEpisodeGoal(typeof analyzedGoal === 'string' ? analyzedGoal : String(analyzedGoal || ''))

          // STEP 2: Generate beat sheet with the analyzed goal
          const beatController = new AbortController()
          const beatTimeoutId = setTimeout(() => beatController.abort(), 360000) // 6 minutes (increased for Safari compatibility)
          ...
          const beatResponse = await fetch('/api/generate/beat-sheet', {
            ...
            body: JSON.stringify({
              ...
              allPreviousEpisodes: previousEpisodesArray
            }),
          ...
          if (beatData.success && beatData.beatSheet) {
            ...
            console.log(`✅ Rewrite auto-generation complete for Episode ${episodeNumber}`)
          } else {
        ...
        console.error('Rewrite auto-generation error:', error)
        setBeatSheetGen({ isGenerating: false, error: null, generatedBeats: '' })

        let userMessage = 'Failed to auto-generate rewrite content'
        if (error instanceof Error) {
          userMessage = `Rewrite failed: ${error.message}`
        }
        ...
        setRewriteEpisodeNumber(null)
      }
    }

    performRewrite()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isRewrite, allEpisodes, storyBible, storyBibleId, episodeNumber])
  ...
  const handleGenerateBeatSheet = async (goalOverride?: string) => {
    ...
    try {
      // Create AbortController with 6 minute timeout (Azure can be slow, Safari needs more time)
      const controller = new AbortController()
      const timeoutId = setTimeout(() => controller.abort(), 360000) // 6 minutes (increased for Safari compatibility)
  ...
  const doWriteScript = async () => {
    ...
    try {
      ...
      // Create AbortController with extended timeout for episode generation (Safari needs more time)
      const episodeController = new AbortController()
      const episodeTimeoutId = setTimeout(() => episodeController.abort(), 600000) // 10 minutes (increased for Safari compatibility)
      ...
      let errorMessage = error instanceof Error ? error.message : 'Failed to generate script'
      
      // Provide more specific error message for timeout
      if (error instanceof Error && (error.name === 'AbortError' || error.message.includes('aborted'))) {
        errorMessage = 'Generation timed out. Please try again. Safari may need more time for complex generations.'
      }
  ...
  const doSurpriseMe = async () => {
    ...
    console.log('✨ SURPRISE ME: Analyzing story for intelligent settings...')
      ...
      if (analysisData.success && analysisData.settings) {
        ...
        // STEP 2: Generate beat sheet with the analyzed goal
        // Create AbortController with 6 minute timeout for beat sheet (increased for Safari compatibility)
        const beatController = new AbortController()
        const beatTimeoutId = setTimeout(() => beatController.abort(), 360000) // 6 minutes
  ...
  // Rewrite Episode Function - Navigate to episode studio and generate new content
  const handleRewriteEpisode = async (targetEpisodeNumber: number) => {
    if (!storyBible || !storyBibleId) {
      alert('Story bible is required to rewrite an episode')
      return
    }

    const episodeToRewrite = allEpisodes[targetEpisodeNumber]
    if (!episodeToRewrite) {
      alert(`Episode ${targetEpisodeNumber} not found`)
      return
    }

    // Navigate to the target episode's studio page
    const studioUrl = `/episode-studio/${targetEpisodeNumber}?storyBibleId=${storyBibleId}${previousChoice ? `&previousChoice=${encodeURIComponent(previousChoice)}` : ''}&rewrite=true`
        ...
        {showCheatSheet && storyBible && (
          <>
            ...
            <div className="p-6">
                  ...
                  <div className="space-y-2">
                    {previousEpisode.episodeTitle && (
                      <p className="text-[#e7e7e7] font-semibold text-sm">
                        Episode {episodeNumber - 1}: {typeof previousEpisode.episodeTitle === 'string' ? previousEpisode.episodeTitle : String(previousEpisode.episodeTitle || '')}
                      </p>
                    )}
                    ...
                    {storyBible.mainCharacters.map((char: any, i: number) => (
                      <div key={i} className="bg-[#2a2a2a] rounded-lg p-3">
                        <p className="font-semibold text-[#10B981] text-sm">
                          {typeof char.name === 'string' ? char.name : String(char.name || 'Unknown')}
                        </p>
                        {char.premiseFunction && (
                          <p className="text-[#e7e7e7]/60 text-xs mt-1">
                            {typeof char.premiseFunction === 'string' ? char.premiseFunction : String(char.premiseFunction)}
                          </p>
                        )}
                        {char.psychology && (
                          <>
                            {char.psychology.want && typeof char.psychology.want === 'string' && (
                          <p className="text-[#e7e7e7]/50 text-xs mt-1">
                            <span className="text-blue-400">Want:</span> {char.psychology.want}
                          </p>
                            )}
                            {char.psychology.goal && typeof char.psychology.goal === 'string' && (
                              <p className="text-[#e7e7e7]/50 text-xs mt-1">
                                <span className="text-blue-400">Goal:</span> {char.psychology.goal}
                              </p>
                            )}
                            {char.psychology.obstacle && typeof char.psychology.obstacle === 'string' && (
                              <p className="text-[#e7e7e7]/50 text-xs mt-1">
                                <span className="text-blue-400">Obstacle:</span> {char.psychology.obstacle}
                              </p>
                            )}
                            {char.psychology.motivation && typeof char.psychology.motivation === 'string' && (
                              <p className="text-[#e7e7e7]/50 text-xs mt-1">
                                <span className="text-blue-400">Motivation:</span> {char.psychology.motivation}
                              </p>
                            )}
                          </>
                        )}
              ...
              {/* Rewrite Episode Section */}
              {Object.keys(allEpisodes).length > 0 && (
                <div className="mt-6 pt-4 border-t border-[#36393f]">
                  <h4 className="text-sm font-semibold text-[#10B981]/70 mb-3">🔄 Rewrite Episode</h4>
                  <p className="text-xs text-[#e7e7e7]/60 mb-3">
                    Select a previous episode to open its Episode Studio and generate a new episode goal, beat sheet, and director's notes.
                  </p>
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {Object.values(allEpisodes)
                      .sort((a: any, b: any) => {
                        const aNum = a.episodeNumber || 0
                        const bNum = b.episodeNumber || 0
                        return bNum - aNum // Show newest first
                      })
                      .map((ep: any) => {
                        const epNum = ep.episodeNumber || 0
                        // Safely convert to string, handle objects
                        let epTitle = `Episode ${epNum}`
                        if (ep.episodeTitle) {
                          epTitle = typeof ep.episodeTitle === 'string' ? ep.episodeTitle : String(ep.episodeTitle)
                        } else if (ep.title) {
                          epTitle = typeof ep.title === 'string' ? ep.title : String(ep.title)
                        }
                        const isCurrentlyRewriting = isRewriting && rewriteEpisodeNumber === epNum
                        
                        return (
                          <button
                            key={epNum}
                            onClick={() => handleRewriteEpisode(epNum)}
                            disabled={beatSheetGen.isGenerating || scriptGen.isGenerating}
                            className={`w-full text-left bg-[#2a2a2a] border rounded-lg p-3 hover:bg-[#36393f] transition-all duration-200 ${
                              'border-[#36393f] hover:border-[#10B981]/50'
                            } ${
                              beatSheetGen.isGenerating || scriptGen.isGenerating
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-[#e7e7e7]/80 truncate">
                                  {epTitle}
                                </p>
                              </div>
                              {!beatSheetGen.isGenerating && !scriptGen.isGenerating && (
                                <span className="text-[#10B981] text-xs ml-2">→</span>
                              )}
                            </div>
                          </button>
                        )
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (data.success && data.analysis) {
                          ...
                          // Fill all form fields with AI analysis (ensure strings)
                          const goalText = analysis.episodeGoal || option.text
                          setEpisodeGoal(typeof goalText === 'string' ? goalText : String(goalText || ''))
                          ...
                          const notesText = analysis.directorsNotes || `Building on: "${option.text}"`
                          setDirectorsNotes(typeof notesText === 'string' ? notesText : String(notesText || ''))
