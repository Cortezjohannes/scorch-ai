...
interface ArcPreProductionSelectorProps {
  isOpen: boolean
  onClose: () => void
  storyBible: any
  storyBibleId: string
...
export default function ArcPreProductionSelector({
  isOpen,
  onClose,
  storyBible,
  storyBibleId,
  ...
  const handleSelect = (arcIndex: number) => {
    router.push(`/preproduction/arc/${arcIndex}?storyBibleId=${storyBibleId}`)
    onClose()
  }
          ...
          <div className={`p-6 border-b ${prefix}-border flex items-center justify-between`}>
            <div>
              <h2 className={`text-xl font-bold ${prefix}-text-primary`}>
                Select Arc for Production Assistant
              </h2>
              <p className={`text-sm mt-1 ${prefix}-text-secondary`}>
                Choose an arc to view or create production materials
              </p>
            </div>
            <button
              onClick={onClose}
